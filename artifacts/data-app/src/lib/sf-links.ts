...
//
// When they are unset (the public demo build) every link resolves SAME-ORIGIN
// (empty base → relative `/<id>` and `/lightning/...` URLs), where the demo
// api-server mounts fake Salesforce record pages (see api-server
// src/lib/fake-crm.ts). The UI keeps its "open in Salesforce" affordances
// without exposing anything real, and clicks land on a dummy in-app page
// instead of a dead external domain.
