...
export default function Dashboard({ authUser, realUser, allowImpersonate }: DashboardProps) {
  ...
  const calendarDefaultMonth = useMemo(
    () => { const t = getTodayPST(); return new Date(t.getFullYear(), t.getMonth() - 1); },
    [],
  );
      ...
      <main className="flex-1 overflow-y-auto mt-[160px] p-4 lg:p-6 print:mt-0">
        <div className="max-w-[1600px] mx-auto">
          {pipelineQuery.isError && (
            <div className="mb-4 flex items-center justify-between gap-4 rounded-lg border border-red-300 bg-red-50 dark:bg-red-950/40 dark:border-red-800 px-4 py-3 print:hidden">
              <div className="flex items-center gap-2 text-[13px] text-red-700 dark:text-red-300">
                <AlertTriangle className="w-4 h-4 shrink-0" />
                <span>
                  Couldn't load dashboard data — the server returned an error. The numbers below may be blank or stale.
                </span>
              </div>
              <button
                onClick={() => pipelineQuery.refetch()}
                className="shrink-0 h-[28px] px-3 bg-red-600 hover:bg-red-700 text-white rounded-md text-[11px] font-medium transition-colors"
              >
                Retry
              </button>
            </div>
