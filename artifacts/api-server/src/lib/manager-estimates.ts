...
export async function ensureCurrentMonthRows(
  ...
  try {
    // dbDirect (autocommit pool), NOT the routed `db` handle: these $0 default
    // rows are deterministic system writes, identical for every viewer. In the
    // demo, running them inside a session's never-committed transaction would
    // (a) needlessly mark the session as edited and (b) make every other
    // session's identical ON CONFLICT insert lock-wait behind the uncommitted
    // row — one visitor's open transaction could hang everyone else's
    // dashboard load. Committing them via the pool is safe and shared.
    await dbDirect
